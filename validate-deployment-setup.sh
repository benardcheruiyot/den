...
if [ -f "backend/server.js" ]; then
    ...
    if grep -q "process.env.PORT" backend/server.js; then
        print_success "Port configuration uses environment variable (default 3005)"
    else
        print_warning "Port should be configurable via environment variable (default 3005)"
    fi
else
